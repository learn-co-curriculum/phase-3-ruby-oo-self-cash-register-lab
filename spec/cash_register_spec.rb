...
describe 'CashRegister' do
  ...
  describe '#add_item' do
    it 'accepts a title and a price and increases the total' do
      expect{cash_register.add_item("eggs", 0.98)}.to increase(cash_register.total).by(0.98)
    end

    it 'also accepts an optional quantity' do
      expect{cash_register.add_item("book", 5.00, 3)}.to increase(cash_register.total).by(15.00)
    end
  end
  ...
  describe '#apply_discount' do
    context 'the cash register was initialized with an employee discount' do
      ...
      it 'reduces the total' do
        cash_register.total = 0
        cash_register_with_discount.add_item("macbook air", 1000)
        expect{cash_register_with_discount.apply_discount}.to reduce(cash_register_with_discount.total).by(200)
      end
    end
  ...
  describe '#void_last_transaction' do
    it 'subtracts the last transaction from the total' do
      ...
      expect{cash_register.void_last_transaction}.to reduce(cash_register.total).by(1.76)
    end
  end
end